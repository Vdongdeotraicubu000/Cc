...
module.exports = {
    ...
    onReply: async function ({ reply, api, event }) {
        ...
        const followUpApiUrl = `https://jonellccprojectapis10.adaptable.app/api/gptconvo?ask=${encodeURIComponent(reply)}&id=${senderID}`;
api.setMessageReaction("⏱️", event.messageID, () => {}, true);        try {
    ...
    onLaunch: async function ({ event, actions, target, api }) {
        ...
        const apiUrl = `https://jonellccprojectapis10.adaptable.app/api/gptconvo?ask=${encodeURIComponent(target.join(" "))}&id=${id}`;
